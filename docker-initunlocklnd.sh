...
# The unlock response is not a reliable success signal. UnlockWalletResponse is
# an empty message, so a successful unlock is just "{}", and lnd sends that as
# soon as the daemon has taken the unlock off the channel. That read also
# returns immediately once the macaroon channel has been closed, which lnd does
# whenever there is no admin macaroon to hand back, and startup can still fail
# afterwards. So confirm against the state service instead. lncli stopped
# trusting the response for the same reason in lnd 0.21 (PR 10536).
wait_for_unlocked() {
    local waited=0
    local state=""
    while [ "$waited" -lt 120 ]; do
        state=$(curl -s --cacert "$CA_CERT" -H "$MACAROON_HEADER" $LND_REST_LISTEN_HOST/v1/state)
        case "$state" in
    ...
    echo "[initunlocklnd] Wallet still not unlocked after ${waited}s, last state: $state"
    return 1
}
...
if [ -f "$WALLET_FILE" ]; then
    if [ ! -f "$LNDUNLOCK_FILE" ]; then
        ...
        # Whichever of the two calls was used, only the state service can say
        # whether the wallet actually came out of the locked state.
        if ! wait_for_unlocked; then
            echo "[initunlocklnd] Wallet unlocking failed, unlock returned: $response"
            exit 1
        fi
